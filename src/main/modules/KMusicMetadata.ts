...
export class KMusicMetadata extends KModule {
  ...
  private async loadMusicLyrics(path: string): Promise<ILyric[]> {
    ...
    if (detext) {
      return this.formatLyrics(detext, ext)
    } else {
   ...
   * @returns 格式化后的歌词列表
   */
  private formatLyrics(text: string, ext: 'lrc' | 'vtt'): ILyric[] {
   ...
   * @returns 图片的主色调
   */
  private async getPictureMainColor(buffer: Uint8Array): Promise<string> {
   ...
   * @param res 结果列表
   */
  private medianCut(cube: number[][], res: { rank: number, color: Color }[]): void {
    // 限定颜色立方体的最大尺寸
    const quality = 100
    ...
    if (r < quality && g < quality && b < quality) {
      ...
      this.medianCut(left, res)
      this.medianCut(right, res)
    }
  }
   ...
   * @returns 图片的主色调
   */
  private async getPictureMainColorByMap(buffer: Uint8Array): Promise<string> {
